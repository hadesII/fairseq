...
MANIFEST_COLUMNS = [ "audio", "n_frames", "tgt_text", "speaker"]
...
class MUSTC(Dataset):
    ...
    # LANGUAGES = ["cantonese","hangzhouhua", "hunanhua", "jiangxihua", "sichuanhua", "wuhanhua"]
    LANGUAGES = ["cantonese"]

    def __init__(self, root: str, lang: str, split: str) -> None:
        assert split in self.SPLITS and lang in self.LANGUAGES
        _root = Path(root)
        wav_root, txt_root = _root / "wav" / lang, _root / "txt" / lang / "data"
    ...
    def __getitem__(self, n: int) -> Tuple[torch.Tensor, int, str,str]:
        wav_path,  sr, src_utt,u_id = self.data[n]
        waveform, _ = get_waveform(wav_path)
        waveform = torch.from_numpy(waveform)
        waveform = waveform.tolist()
        return waveform, sr, src_utt,u_id
...
def process(args):
    ...
    for lang in MUSTC.LANGUAGES:
        cur_root = root / "txt" / f"{lang}"
        if not cur_root.is_dir():
            print(f"{cur_root.as_posix()} does not exist. Skipped.")
            continue
        # Extract features
        feature_root = cur_root / "mel"
        ...
        for split in MUSTC.SPLITS:
            ...
            for waveform, sample_rate, _ ,utt_id in tqdm(dataset):
                features = processor.feature_extractor(waveform, sampling_rate=sample_rate).input_features[0]
        ...
        for split in MUSTC.SPLITS:
            ...
            for wav, sr, src_utt, utt_id in tqdm(dataset):
        ...
        spm_filename_prefix = f"spm_{args.vocab_type}_{args.task}"
        ...
        tokenizer.save_vocabulary(cur_root)
        gen_config_yaml(
            cur_root,
